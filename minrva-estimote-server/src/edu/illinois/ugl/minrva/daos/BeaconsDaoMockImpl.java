...
public class BeaconsDaoMockImpl implements BeaconsDao {
	...
	public List<Beacon> getBeaconsByVersion(long id) {
		...
		List<Beacon> beacons = (List<Beacon>) MockData.instance
				.getBeaconsContent().values();
	...
	@Override
	public boolean createBeacon(long vId, int uuid, int major, int minor,
			int x, int y, int z) {
	...
	@Override
	public boolean updateBeacon(long vId, int uuid, int major, int minor,
			int x, int y, int z) {
		String bId = MockData.instance.getBeaconId(vId, uuid, major, minor);
